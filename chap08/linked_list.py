...
class LinkedList:
    ...
    def search(self, data: Any) -> int:
        """data와 값이 같은 노드를 검색"""
        cnt = 0 # 몇 번째 원소를 스캔하고 있는지를 나타내는 카운터용 변수
        ptr = self.head
        while ptr is not None: # ptr이 꼬리 노드까지 가지 않을 때까지
            if ptr.data == data:
                self.current = ptr
                return cnt
            cnt += 1
    ...
    def add_first(self, data: Any) -> None:
        ...
        self.head = self.current = Node(data, ptr)  # 삽입할 노드 생성, 뒤쪽 포인터가 참조하는 곳은 기존의 self.head
        self.no += 1
    ...
    def __iter___(self) -> LinkedListIterator:
        """이터레이터를 반환"""
