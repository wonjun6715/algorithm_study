...
class BinarySearchTree:
    ...
    def add(self, key: Any, value: Any) -> bool:
        ...
        def add_node(node: Node, key: Any, value: Any) -> None:
            ...
            elif key < node.key: # key가 node의 key보다 작으면
                if node.left is None: # 왼쪽 포인터가 비어 있으면
                    node.left = Node(key, value, None, None) # 삽입
                else: # 왼쪽 포인터가 비어 있지 않으면 
                    add_node(node.left, key, value) # 주목 노드를 왼쪽 자식 노드로 옮김
            else: # key가 node의 key보다 크면
                if node.right is None: # 오른쪽 포인터가 비어 있으면
                    node.right = Node(key, value, None, None) # 삽입
                else: # 오른쪽 포인터가 비어 있지 않으면
                    add_node(node.right, key, value) # 주목 노드를 오른쪽 자식 노드로 옮김
            return True
    ...
    def dump(self) -> None:
        ...
        def print_subtree(node: Node):
            """node를 루트로 하는 서브트리의노드를 키의 오름차순으로 출력"""
            if node is not None:
